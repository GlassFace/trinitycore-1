...
static const char* GetStatName(uint32 ItemStatType)
{
    ...
    case ITEM_MOD_DODGE_RATING             	: return "闪躲"; break;
    case ITEM_MOD_PARRY_RATING             	: return "招架"; break;
    case ITEM_MOD_HIT_RATING               	: return "命中"; break;
    case ITEM_MOD_CRIT_RATING              	: return "暴击"; break;
    case ITEM_MOD_HASTE_RATING             	: return "急速."; break;
    case ITEM_MOD_EXPERTISE_RATING         	: return "专业."; break;
    default: return NULL;
    }
}
...
static const char* GetSlotName(uint8 slot, WorldSession* session)
{
    ...
    case EQUIPMENT_SLOT_HANDS     : return "手部";
    case EQUIPMENT_SLOT_FINGER1   : return "右手戒指";
    case EQUIPMENT_SLOT_FINGER2   : return "左手戒指";
    case EQUIPMENT_SLOT_TRINKET1  : return "右边饰品";
    case EQUIPMENT_SLOT_TRINKET2  : return "左边饰品";
...
void RemoveReforge(Player* player, uint32 itemguid, bool update)
{
	...
	Item* invItem = player->GetItemByGuid(MAKE_NEW_GUID(itemguid, 0, HIGHGUID_ITEM));
    if (invItem)
...
public:
    ...
    {
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "选择一个装备重铸:", 0, Melt(MAIN_MENU, 0));
        ...
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "刷新菜单", 0, Melt(MAIN_MENU, 0));
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 melt) override
    {
        ...
        case SELECT_STAT_REDUCE:
            ...
            if (Item* invItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, action))
            {
                if (IsReforgable(invItem, player))
                {
                    uint32 guidlow = invItem->GetGUIDLow();
                    const ItemTemplate* pProto = invItem->GetTemplate();
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "属性减少:", sender, melt);
                    for (uint32 i = 0; i < pProto->StatsCount; ++i)
                    {
                ...
                {
                    player->GetSession()->SendNotification("无效项目");
        ...
        case SELECT_STAT_INCREASE:
            ...
            // action = StatsCount id
            {
                ...
                if (invItem)
                {
                    ...
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "统计增加:", sender, melt);
        ...
        case SELECT_RESTORE:
            {
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_BATTLE, "选择要移除的部位:", sender, melt);
                if (!player->reforgeMap.empty())
                {
                    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
                    {
                        if (Item* invItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
                            if (player->reforgeMap.find(invItem->GetGUIDLow()) != player->reforgeMap.end())
                                if (const char* slotname = GetSlotName(slot, player->GetSession()))
                                    player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_INTERACT_1, slotname, invItem->GetGUIDLow(), Melt(RESTORE, 0), "移除幻化\n\n" + invItem->GetTemplate()->Name1, 0, false);
                    }
                }
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "刷新", sender, melt);
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "返回..", 0, Melt(MAIN_MENU, 0));
                player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
            }
            break;
                ...
                if (menu < stat_type_max)
                {
                    ...
                    if (invItem && IsReforgable(invItem, player))
                    {
                        if (player->HasEnoughMoney(invItem->GetTemplate()->SellPrice < (10 * GOLD) ? (10 * GOLD) : invItem->GetTemplate()->SellPrice))
                        {
                            ...
                            player->GetSession()->SendNotification("穷逼.");
                    }
                    else
                        player->GetSession()->SendNotification("无效项目.");
                }
