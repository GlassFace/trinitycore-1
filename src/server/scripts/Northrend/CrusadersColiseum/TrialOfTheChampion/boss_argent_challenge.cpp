...
enum Spells
{
    ...
    SPELL_HAMMER__BAR   = 66904, // s players cast bar
        ...
        class spell_eadric_hoj_SpellScript: public SpellScript
        {
            ...
            void HandleOnHit()
            {
                if (GetHitUnit() && GetHitUnit()->GetTypeId() == TYPEID_PLAYER)
                    ...
                    {
                        SetHitDamage(0);
                        GetHitUnit()->AddAura(SPELL_HAMMER__BAR, GetHitUnit());
                    }
    ...
    struct boss_paletressAI : public BossAI
    {
        ...
        void DamageTaken(Unit* /*done_by*/, uint32 &damage) 
        {
            if (damage >= me->GetHealth())
            {
                ...
                me->setFaction(35);
                bDone = true;
                if (GameObject* pGO = ObjectAccessor::GetGameObject(*me, pInstance->GetData64(DATA_MAIN_GATE)))
                        pInstance->HandleGameObject(pGO->GetGUID(),true);	
                if (GameObject* pGO = ObjectAccessor::GetGameObject(*me, pInstance->GetData64(DATA_MAIN_GATE1)))
                        pInstance->HandleGameObject(pGO->GetGUID(),true);		
