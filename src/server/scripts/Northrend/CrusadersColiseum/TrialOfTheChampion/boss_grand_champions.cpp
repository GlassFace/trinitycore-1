...
bool GrandChampionsOutVehicle(Creature* me)
{
    ...
    Creature* pGrandChampion1 = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_GRAND_CHAMPION_1));
    Creature* pGrandChampion2 = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_GRAND_CHAMPION_2));
    Creature* pGrandChampion3 = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_GRAND_CHAMPION_3));
        ...
        void Reset()
        {
		    ...
		    if (hasBeenInCombat && pMap && pMap->IsDungeon())
            {
			    ...
			    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
			    {
			     ...
			     if(pInstance)
			     {
				    GameObject* GO = ObjectAccessor::GetGameObject(*me, pInstance->GetData64(DATA_MAIN_GATE1));
			     if(GO)
